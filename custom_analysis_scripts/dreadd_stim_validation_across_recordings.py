"""Standalone validation across recordings for c21 vs media conditions."""
...
def label_group(rec_name: str) -> str:
    """Map recording folder name to - C21/+ C21/Unknown based on substrings."""
    name = rec_name.lower()
    if "media" in name or "-c21" in name or "noc21" in name:
        return "- C21"
    if "c21" in name or "+c21" in name or "cn21" in name:
        return "+ C21"
    return "Unknown"
...
def drop_outliers(df: pd.DataFrame, cols: list[str]) -> pd.DataFrame:
    """Drop rows with outliers (1.5*IQR) per group (group + smoothed) for selected columns."""
    if df.empty:
        return df
...
def make_pre_post_movie(pre_mc: Path, post_mc: Path, out_dir: Path, fps: float) -> None:
    """Create side-by-side movies with shared normalization and colorbar."""
    pre = load_mc_movie(pre_mc)
    post = load_mc_movie(post_mc)
    if pre.shape != post.shape:
        raise ValueError(f"Movies must match shape, got {pre.shape} vs {post.shape}")
    ...
    try:
        ...
        cb.set_label("Intensity (normalized to +C21, 1st–99th pct)", fontsize=9)
        fig.savefig(colorbar_path, dpi=200, bbox_inches="tight")
        plt.close(fig)
        print(f"[dreadd_stim_across] Wrote colorbar: {colorbar_path}")
    except Exception as e:  # noqa: BLE001
        print(f"[dreadd_stim_across] Skipping colorbar: {e}")
    ...
    def write_movie(out_path: Path, fps_value: float) -> None:
        ...
        try:
            ...
            print(f"[dreadd_stim_across] Wrote movie: {out_path}")
        except Exception as e:  # noqa: BLE001
            print(f"[dreadd_stim_across] Skipping movie: {e}")

    write_movie(out_dir / "media_vs_c21_motion_corrected.mp4", fps_value=fps)
    write_movie(out_dir / "media_vs_c21_motion_corrected_fast.mp4", fps_value=fps * FAST_FPS_MULTIPLIER)


def boxplot_counts(df: pd.DataFrame, title: str, out_path: Path) -> None:
    """Boxplot + jittered points for peak counts."""
    if df.empty:
        print(f"[dreadd_stim_across] Skipping plot (empty data): {out_path}")
        return
    ...
    for grp in ["- C21", "+ C21"]:
        ...
        if vals:
            ...
            labels.append(grp)
    if not data:
        print(f"[dreadd_stim_across] Skipping plot (no groups with data): {out_path}")
        return
    ...
    for xpos, lab in zip(x_positions, labels):
        ...
        scatter_col = COL_MEDIA if lab == "- C21" else COL_C21
        plt.scatter(
    ...
    plt.close()
    print(f"[dreadd_stim_across] Wrote plot: {out_path}")
...
def multi_panel_boxplots(
    ...
    for row_idx, (label, df) in enumerate(dfs):
        for col_idx, (metric, title) in enumerate(metrics):
            ...
            data = []
            labels = []
            for grp, col in (("- C21", COL_MEDIA), ("+ C21", COL_C21)):
...
def run_stats(df: pd.DataFrame, metric_cols: list[str], smoothed_flag: bool) -> list[dict]:
    ...
    for metric in metric_cols:
        ...
        try:
            ...
            print(f"[dreadd_stim_across] Stats failed for {metric} (smoothed={smoothed_flag}): {e}")
            continue
        results.append(
            {
                "metric": metric,
...
def main() -> None:
    ...
    print(f"[dreadd_stim_across] Processing project root: {PROJECT_ROOT}")
    outputs = process_project_root(PROJECT_ROOT, fps=FPS, config=cfg)
    print(f"[dreadd_stim_across] Recordings processed: {len(outputs)}")
    ...
    media_keys = [k for k in manifests if label_group(k) == "- C21"]
    c21_keys = [k for k in manifests if label_group(k) == "+ C21"]
    if media_keys and c21_keys:
        media_manifest = manifests[media_keys[0]]
        c21_manifest = manifests[c21_keys[0]]
        media_mc = json.loads(media_manifest.read_text())["paths"]["motion_corrected_tiff"]
        c21_mc = json.loads(c21_manifest.read_text())["paths"]["motion_corrected_tiff"]
        movie_dir = PROJECT_ROOT / "roi_analysis_contract_summary"
        make_pre_post_movie(Path(media_mc), Path(c21_mc), movie_dir, fps=FPS)
    else:
        print("[dreadd_stim_across] Skipping movies (could not find both groups)")

    df_unsmoothed = pd.DataFrame(rows_counts)
    df_smoothed = pd.DataFrame(rows_counts_smoothed)
    print(f"[dreadd_stim_across] ROI entries before smoothing/outlier filter: unsmoothed={len(df_unsmoothed)}, smoothed={len(df_smoothed)}")
    ...
    print(f"[dreadd_stim_across] ROI entries after count outlier filter: {len(summary_counts)}")
    ...
    metrics_unsmoothed_df = drop_outliers(metrics_unsmoothed_df, FILTER_METRICS)
    metrics_smoothed_df = drop_outliers(metrics_smoothed_df, FILTER_METRICS)
    print(f"[dreadd_stim_across] ROI entries after metric outlier filter: unsmoothed={len(metrics_unsmoothed_df)}, smoothed={len(metrics_smoothed_df)}")
    # Report non-NaN counts per metric for diagnostics.
    for metric in metric_cols:
        u_count = metrics_unsmoothed_df[metric].notna().sum()
        s_count = metrics_smoothed_df[metric].notna().sum()
        print(f"[dreadd_stim_across] Non-NaN {metric}: unsmoothed={u_count}, smoothed={s_count}")
    metrics_all = pd.concat([metrics_unsmoothed_df, metrics_smoothed_df], ignore_index=True)

    stats_rows = run_stats(metrics_unsmoothed_df, metric_cols, smoothed_flag=False) + run_stats(
        metrics_smoothed_df, metric_cols, smoothed_flag=True
    )
